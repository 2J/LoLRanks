...
//------------------------------------------------------------------------------------------------//
    ...
    public function actionAccount()
    {
		$id = $this->user->id;
        // get user details
        $user = $model = User::findOne($id);
		$user->scenario = 'account';
		
		if (Yii::$app->request->isAjax && $user->load($_POST))
		{
        ...
        {
			$newdetails = array('email' => $user->email, 'password' => $user->password);
			$user->attributes = $user->oldAttributes;
			
			//Validate old password TODO: move this to model
			if(!$user->validatePassword($user->password_old)){
				$user->email = $newdetails['email'];
				$user->password = NULL;
				$user->password_old = NULL;
				$user->password_confirm = NULL;
				$user->addError('password_old','Wrong password');
